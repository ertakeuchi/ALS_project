...
outDir <- file.path(homeDir, "experimental_record/230803_experiment1/out_1")
...
mainDir <- file.path(homeDir, "R/RData", tissue)
...
for (no in seq_along(file_data$project)){
  ...
  if(file_data$facs[no] == TRUE){
    ...
    so_filtered@meta.data <- so_filtered@meta.data %>%
      dplyr::mutate(SubjectID = case_when(
        ...
        TRUE ~ as.character(MULTI_ID)  # それ以外の場合はMULTI_IDの値をそのまま使用
      ))
  ...
  #------------------------------------------------------------------------------------------------#
    if (file_data$demuxlet[no] == TRUE){
      ...
      # Select cells which are matched between RNA and ATAC on DEMUXLET
  ...
  #------------------------------------------------------------------------------------------------#
    ...
    saveRDS(so.QC, file = file.path(mainDir, paste0(file_data$project[no], "_so.rds")))
